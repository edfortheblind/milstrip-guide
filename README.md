...
Version **1.3.0** · Evidence dated **September 24, 2026**.
...
**Both apps are published.** Stage passed Studio runtime tests against local
PostgreSQL. The current account's player access is blocked by licensing, which
IT is handling. Prod remains disabled until its independent database is configured.
Production handoff and downstream receipts remain Phase 2 work: Azure SQL first,
then accepted PostgreSQL production. Screens contain synthetic data.
